setwd("G:/Researchers-Investigators/Berg/BaseCodeQRSAE/BaseCodeSimQRSAE")
rm(list=ls(all=TRUE))
library(nlme)
library("lqmm")
library("sae")
...
source("transq1.R")
source("bootestsimplesortfixtrans.R")
source("predTransEBFast.R")
...
if(doBoot){
  ...
  repeat{
    ...
    bootestyxq <- bootestsimple2FixSortTrans(data.frame(Y = bootsampyxq[[1]], X = lxN[bootsampyxq[[2]]]), X = lxN[bootsampyxq[[2]]], areafac.pop[bootsampyxq[[2]]], D, GN[bootsampyxq[[2]],], tauvec,  1500, lxN, b.dist, areafac.pop, bootsampyxq[[2]], use.cl = FALSE, trunc = FALSE, lam = lamcur)
  ...
  if(cnt%%10 == 0){ save.image("Boot5318/SNTransBoot3Normal.Rdata") }	
...
(mean(apply( mhb25s, 2, mean)) - mean(apply((qhats.pop25  - w25Js)^2, 2, mean)))/ mean(apply((qhats.pop25  - w25Js)^2, 2, mean))*100

mean(apply((w10JCs[1:100,] - qhats.pop10[1:100,])^2, 2, mean))
mean(apply((EB.norm.10s[1:100,] - qhats.pop10[1:100,])^2, 2, mean))
mean(apply((q10bs[1:100,] - qhats.pop10[1:100,])^2, 2, mean))

mean(apply((w25JCs[1:100,] - qhats.pop25[1:100,])^2, 2, mean))
mean(apply((EB.norm.25s[1:100,] - qhats.pop25[1:100,])^2, 2, mean))
mean(apply((q25bs[1:100,] - qhats.pop25[1:100,])^2, 2, mean))

mean(apply((w25JCs[1:100,] - qhats.pop25[1:100,]), 2, mean))
mean(apply((EB.norm.25s[1:100,] - qhats.pop25[1:100,]), 2, mean))
mean(apply((q25bs[1:100,] - qhats.pop25[1:100,]), 2, mean))

mean(apply( mhb25s[1:100,], 2, mean))

mean(apply((w50JCs[1:100,] - qhats.pop5[1:100,])^2, 2, mean))
mean(apply((EB.norm.50s[1:100,] - qhats.pop5[1:100,])^2, 2, mean))
mean(apply((q5bs[1:100,] - qhats.pop5[1:100,])^2, 2, mean))
mean(apply( mhb50s[1:100,], 2, mean))

mean(apply((w90JCs[1:100,] - qhats.pop90[1:100,])^2, 2, mean))
mean(apply((EB.norm.90s[1:100,] - qhats.pop90[1:100,])^2, 2, mean))
mean(apply((q90bs[1:100,] - qhats.pop90[1:100,])^2, 2, mean))


par(mfrow = c(1,2))
hist((w25JCs[1:100,] - qhats.pop25[1:100,])[,2])
hist((EB.norm.25s[1:100,] - qhats.pop25[1:100,])[,2])
